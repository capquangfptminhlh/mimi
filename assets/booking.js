(() => {
  ...
  const renderLastRequest = () => {
    ...
    box.innerHTML=`<h3>Yêu cầu gần nhất</h3><span class="request-code">${item.code}</span><p><strong>${item.title}</strong></p><p>${item.when}</p><p>Trạng thái: <strong>Chờ Lumi xác nhận</strong></p>`;
  };
  const showResult = (form, data) => {
    ...
    saveRequest(data);
    const live=$('[data-booking-live]'); if(live) live.textContent=`Đã tạo yêu cầu ${data.code}. Vui lòng gửi SMS hoặc gọi Lumi để xác nhận.`;
  };
  ...
  const appointmentSubmit = form => {
    ...
    showResult(form,{code,title:service,when:`${humanDate(date)} · ${time}`,message,createdAt:new Date().toISOString()});
  };
  const hotelSubmit = form => {
    ...
    showResult(form,{code,title:`Pet Hotel · ${nights} đêm`,when:`${humanDate(inDate)} → ${humanDate(outDate)}`,message,createdAt:new Date().toISOString()});
  };
  const updateHotelEstimate = form => {
    ...
    const start=new Date(`${inDate}T${inTime}`),end=new Date(`${outDate}T${outTime}`);
    if(end<=start){box.textContent='Thời gian trả bé cần sau thời gian nhận bé.';return;}
