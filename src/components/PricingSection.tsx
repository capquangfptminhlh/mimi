        ...
        <div className="bg-white rounded-[32px] shadow-xl border-2 border-amber-100 p-6 md:p-8 max-w-4xl mx-auto mb-14 relative overflow-hidden">
          ...
          <div className="mt-8 pt-6 border-t border-orange-50 bg-amber-50/30 rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-shrink-0">
              <button
                onClick={() => {
                  const serviceName = calcService === 'hotel' 
                    ? 'Khách Sạn (Hotel)' 
                    : calcService === 'tvs' 
                      ? 'Combo Tắm Vệ Sinh 10 Bước' 
                      : calcService === 'ctc' 
                        ? 'Combo Tắm Cạo Lông' 
                        : 'Combo Tắm Cắt Tạo Kiểu';
                  const petTypeName = calcPetType === 'dog' ? 'Chó cưng' : 'Mèo cưng';
                  const priceText = computedPrice === -1 ? 'cần báo giá thêm' : formatVND(computedPrice);

                  const message = `Chào Lumi Pet, mình cần tư vấn dịch vụ:\n- Dịch vụ: ${serviceName}\n- Thú cưng: ${petTypeName} (nặng ${calcWeight}kg)\n- Ước tính: ${priceText}`;

                  navigator.clipboard.writeText(message).then(() => {
                    alert('Đã copy thông tin tư vấn! Bạn hãy nhấn Dán (Ctrl+V / Cmd+V) vào khung chat Zalo để gửi cho Lumi Pet nhé.');
                    window.open('https://zalo.me/0989979675', '_blank');
                  }).catch(() => {
                    window.open('https://zalo.me/0989979675', '_blank');
                  });
                }}
                className="bg-sky-600 hover:bg-sky-700 text-white font-extrabold text-[13px] px-5 py-3.5 rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5 text-center cursor-pointer"
              >
