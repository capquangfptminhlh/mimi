...
export default function App() {
  ...
  const handleBookingSubmit = (e: React.FormEvent) => {
    ...
    triggerToast('Đặt lịch thành công! Đang chuyển hướng sang Zalo để bạn gửi xác nhận...');

    // Generate message for Zalo
    const bookingMessage = `🎯 ĐẶT LỊCH MỚI: ${newBooking.id}\n👤 Khách hàng: ${newBooking.customerName}\n📞 SĐT: ${newBooking.customerPhone}\n🐾 Thú cưng: ${newBooking.petName} (${newBooking.petType} - ${newBooking.petWeight}kg)\n🛒 Dịch vụ: ${newBooking.serviceType}\n💰 Ước tính: ${newBooking.totalPrice.toLocaleString('vi-VN')}đ\n📝 Ghi chú: ${newBooking.notes || 'Không có'}\n\nVui lòng kiểm tra và xác nhận lịch giúp mình nhé!`;

    navigator.clipboard.writeText(bookingMessage).then(() => {
      alert('Đã lưu thông tin lịch hẹn! Bạn hãy nhấn Dán (Ctrl+V) vào khung chat Zalo để gửi xác nhận cho Lumi Pet nhé.');
